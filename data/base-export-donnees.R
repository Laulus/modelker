...
setwd(dir = "C:/Users/laulus/Documents/Mirror/THESE/MODELE-COMPLET/aulus/model-new")
...
# garder quand même les écailles et le lecteur car pour filtrer je vais en avoir besoin

# On ne garde que les poissons lus et mesurés, ie dans la colonne lecteur il y a une nom
...
# il faut garder que les poissons où on a une age.tot déterminé
...
# problème il y a des RM manquants
# bdd<-subset(bdd,RM!="NA")
# problème il y a des RT manquants
bdd<-subset(bdd,RT!="NA")
# problème il y a des écailles régénérées R1
bdd<-subset(bdd,R1!="NA")
# problème il y a des agetot ou on a pas le radius ...
...
# bdd<-bddi
bdd<-bddi[sample(1:nrow(bddi),200,replace=FALSE),] # on fait un sample de 20 lignes au hasard
...
# il y a un probleme dans les smolt Age a régler ... tel que coder le poissson est considéré
# comme dispersant réellement l'année suivant son âge à la migration (car RM est compris le plus souvent entre 2 rayons)
...
# sur body size a va etre le même problème que sur smolt
...
bugs.data(data,dir=getwd(),digits=5,data.file="data500ssX.txt")
